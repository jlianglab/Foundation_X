...
from datasets_medical import build_transform_segmentation, PXSDataset, MontgomeryDataset, JSRTClavicleDataset, JSRTHeartDataset,JSRTLungDataset, VinDrRibCXRDataset, ChestXDetDataset
...
def train_one_epoch_SEGMENTATION(model, train_loader, optimizer, loss_scaler, epoch, head_number=None, log_writter_SEGMENTATION=None):
    model.train(True)
    ...
    for idx, (img,mask) in enumerate(train_loader):
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            ...
            out_SegmentationHead = model.module.backbone[0].extra_features_seg(img, head_n=head_number)
        else:
            # out_features, out_classifierHead, out_SegmentationHead = model.backbone(img)
            out_SegmentationHead = model.backbone[0].extra_features_seg(img, head_n=head_number)
...
def main(args):
    ...
    if args.taskcomponent == 'detection':
        print()
        print("-------------")
        print("[Information]  TASK:", "Detection")
        ...
        print("[Information]  Optimizer:", "AdamW")
    ...
    if (not args.resume) and args.pretrain_model_path:
        ...
        print("[Model Info.] Backbone + DINO model weights loaded...")
    ...
    if args.taskcomponent == 'segmentation' or args.taskcomponent == 'segmentation_cyclic':
        ...
        if args.segmentation_dataset == 'jsrt_lung':
            train_image_path_file = [("/data/jliang12/jpang12/dataset/JSRT/All247images", "data/jsrt/train.txt")] # /data/jliang12/jpang12/dataset/JSRT/All247images # /mnt/dfs/jpang12/datasets/JSRT/All247images
            ...
            train_dataset = JSRTHeartDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            val_dataset = JSRTHeartDataset(val_image_path_file,image_size=(img_size,img_size), mode="val")
            val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
                                                     pin_memory=True, shuffle=True,drop_last=False )

            test_dataset = JSRTHeartDataset(test_image_path_file,image_size=(img_size,img_size), mode="val")

            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                      num_workers=num_workers, pin_memory=True,drop_last=False )

        elif args.segmentation_dataset == 'chestxdetdataset':
            ...
            train_dataset = ChestXDetDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            # val_dataset = ChestXDetDataset(val_image_path_file,image_size=(img_size,img_size), mode="val")
            # val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
            #                                          pin_memory=True, shuffle=True,drop_last=False )

            test_dataset = ChestXDetDataset(test_image_path_file,image_size=(img_size,img_size), mode="test")

            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                      num_workers=num_workers, pin_memory=True,drop_last=False )

        
        elif args.segmentation_dataset == 'jsrt_lung_heart_clavicle':
            train_image_path_file = [("/data/jliang12/jpang12/dataset/JSRT/All247images", "data/jsrt/train.txt")]
            val_image_path_file = [("/data/jliang12/jpang12/dataset/JSRT/All247images", "data/jsrt/val.txt")]
            test_image_path_file = [("/data/jliang12/jpang12/dataset/JSRT/All247images", "data/jsrt/test.txt")]

            train_dataset = JSRTLungDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader_jsrtLung = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            val_dataset = JSRTLungDataset(val_image_path_file,image_size=(img_size,img_size), mode="val")
            val_loader_jsrtLung = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
                                                     pin_memory=True, shuffle=True,drop_last=False )

            test_dataset = JSRTLungDataset(test_image_path_file,image_size=(img_size,img_size), mode="val")
            test_loader_jsrtLung = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                      num_workers=num_workers, pin_memory=True,drop_last=False )


            train_dataset = JSRTClavicleDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader_jsrtClavicle = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            val_dataset = JSRTClavicleDataset(val_image_path_file,image_size=(img_size,img_size), mode="val")
            val_loader_jsrtClavicle = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
                                                     pin_memory=True, shuffle=True,drop_last=False )

            test_dataset = JSRTClavicleDataset(test_image_path_file,image_size=(img_size,img_size), mode="val")

            test_loader_jsrtClavicle = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                      num_workers=num_workers, pin_memory=True,drop_last=False )


            train_dataset = JSRTHeartDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader_jsrtHeart = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            val_dataset = JSRTHeartDataset(val_image_path_file,image_size=(img_size,img_size), mode="val")
            val_loader_jsrtHeart = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
                                                     pin_memory=True, shuffle=True,drop_last=False )

            test_dataset = JSRTHeartDataset(test_image_path_file,image_size=(img_size,img_size), mode="val")

            test_loader_jsrtHeart = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                      num_workers=num_workers, pin_memory=True,drop_last=False )
            ...
            train_dataset = VindrCXRHeartDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader_vindrcxrHeart = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            val_dataset = VindrCXRHeartDataset(val_image_path_file,image_size=(img_size,img_size), mode="test")
            val_loader_vindrcxrtHeart = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
                                                     pin_memory=True, shuffle=True,drop_last=False )
            ...
            train_dataset = JSRTLungDataset(train_image_path_file, image_size=(img_size,img_size))
            train_loader_vindrcxrLung = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       num_workers=num_workers, pin_memory=True, shuffle=True,drop_last=True )

            val_dataset = JSRTLungDataset(val_image_path_file,image_size=(img_size,img_size), mode="test")
            val_loader_vindrcxrtLung = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers,
                                                     pin_memory=True, shuffle=True,drop_last=False )
    ...
    if args.taskcomponent == 'classification':
        ...
        if args.classification_dataset == "imagenet":
            ...
            data_dir = "/data/jliang12/jpang12/dataset/nih_xray14/images/images/" #"/mnt/dfs/nuislam/Data/ChestXRay14_images/" ## "/data/jliang12/jpang12/dataset/nih_xray14/images/images/"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.taskcomponent == 'segmentation':
            ...
            train_lossAVG = train_one_epoch_SEGMENTATION(model, train_loader, optimizer, loss_scaler, epoch, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
        ...
        if args.taskcomponent == 'segmentation_cyclic':
            ...
            if args.segmentation_dataset == "jsrt_lung_heart_clavicle":
                ...
                test_y, test_p = test_SEGMENTATION(model, test_loader_jsrtLung, head_number=0, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
                ...
                test_y, test_p = test_SEGMENTATION(model, test_loader_jsrtHeart, head_number=1, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
                ...
                test_y, test_p = test_SEGMENTATION(model, test_loader_jsrtClavicle, head_number=2, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
                ...
                start_time = time.time()
                test_y, test_p = test_SEGMENTATION(model, val_loader_vindrcxrtHeart, head_number=0, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
                ...
                test_y, test_p = test_SEGMENTATION(model, val_loader_vindrcxrtLung, head_number=1, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
