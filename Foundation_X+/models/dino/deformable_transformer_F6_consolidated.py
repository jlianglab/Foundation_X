...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        ## NAD Modified ##
        ## NATE Modified - Consolidate to one decoder ##
        ...
        if (two_stage_type != 'no' and embed_init_tgt) or (two_stage_type == 'no'):
            ...
            ## Nate modified -- create list of three content embeddings ##
            print("[DEBUG] ... creating list of target embeddings ...")
            self.tgt_embed = nn.ModuleList()
            self.tgt_embed.append(nn.Embedding(self.num_queries, d_model)) # 1      # idea: use copy.deepcopy() on these?
            nn.init.normal_(self.tgt_embed[0].weight.data)
            self.tgt_embed.append(nn.Embedding(self.num_queries, d_model)) # 2
            nn.init.normal_(self.tgt_embed[1].weight.data)
            self.tgt_embed.append(nn.Embedding(self.num_queries, d_model)) # 3
            nn.init.normal_(self.tgt_embed[2].weight.data)
            self.tgt_embed.append(nn.Embedding(self.num_queries, d_model)) # 4     
            nn.init.normal_(self.tgt_embed[3].weight.data)
            self.tgt_embed.append(nn.Embedding(self.num_queries, d_model)) # 5
            nn.init.normal_(self.tgt_embed[4].weight.data)
            self.tgt_embed.append(nn.Embedding(self.num_queries, d_model)) # 6
            nn.init.normal_(self.tgt_embed[5].weight.data)
            print(f"[DEBUG]: target embedding list: {self.tgt_embed}")
        else:
            self.tgt_embed = None
        ...
        if two_stage_type =='standard':
            ...
            ## Nate modified -- set up for one encoder ##
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, task_DetHead=None):
        ...
        if self.two_stage_type =='standard':
            ...
            # memory = self.enc_outputMem_LinearProjection[task_DetHead]( memory ) ################# Removed as we only have one decoder
            output_memory, output_proposals = gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes, input_hw)
            # output_memory = self.enc_output_norm[task_DetHead](self.enc_output[task_DetHead](output_memory))      ## Nate modify: only are using one decoder ##
            ...
            enc_outputs_class_unselected = self.enc_out_class_embed[task_DetHead](output_memory)                                                      # Nate modify: class embeds is a list
            enc_outputs_coord_unselected = self.enc_out_bbox_embed[task_DetHead](output_memory) + output_proposals # (bs, \sum{hw}, 4) unsigmoid      # Nate modify: bbox embeds is a list
            ...
            ## Nate modified -- select the target embedding based on the task head ##
            if self.embed_init_tgt:
                # tgt_ = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1) # nq, bs, d_model
                tgt_ = self.tgt_embed[task_DetHead].weight[:, None, :].repeat(1, bs, 1).transpose(0, 1) # nq, bs, d_model
                #print(f"[DEBUG] tgt_.size: {tgt_.size}")
                #print(f"[DEBUG] selecting target... tgt_embed[{task_DetHead}]: {self.tgt_embed[task_DetHead]}")
            else:
                tgt_ = tgt_undetach.detach()

            if refpoint_embed is not None:
                refpoint_embed=torch.cat([refpoint_embed,refpoint_embed_],dim=1)
                tgt_ = tgt_.to(tgt.device)
        ...
        ## Nate modified -- select the target embedding based on the task head ##
            ...
            if self.num_patterns > 0:
                ...
                #print(f"[DEBUG] tgt_embed.size: {tgt_embed.size}")
                tgt_embed[task_DetHead] = tgt.repeat(1, self.num_patterns, 1)
        ...
        # print("[Check] tgt.shape:", tgt.shape)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed[task_DetHead] is not None: ## NAD  -- CHECK THIS for multi Localizer Decoder Implementation
                reference_before_sigmoid = inverse_sigmoid(reference_points)
                delta_unsig = self.bbox_embed[task_DetHead][layer_id](output) # Nate modified to index bbox_embed list
    ...
    def forward_sa(self,
        ...
        if self.self_attn is not None:
            if self.decoder_sa_type == 'sa':
                tgt = tgt.to(tgt_query_pos.device)
