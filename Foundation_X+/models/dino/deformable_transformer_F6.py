...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        if deformable_decoder: # default TRUE
        ...
        ## NAD Modified ##
        decoder_ = []
        decoder_temp = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
        ...
        decoder_.append( copy.deepcopy(decoder_temp) ) # 1
        decoder_.append( copy.deepcopy(decoder_temp) ) # 2
        decoder_.append( copy.deepcopy(decoder_temp) ) # 3
        decoder_.append( copy.deepcopy(decoder_temp) ) # 4
        decoder_.append( copy.deepcopy(decoder_temp) ) # 5
        decoder_.append( copy.deepcopy(decoder_temp) ) # 6
        self.decoder = nn.ModuleList( decoder_ )
        del decoder_temp, decoder_
        ...
        if two_stage_type =='standard':
            ...
            enc_output_temp = []
            enc_output_norm_temp = []

            # self.enc_output = nn.Linear(d_model, d_model)
            # self.enc_output_norm = nn.LayerNorm(d_model)

            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 1
            enc_output_norm_temp.append( copy.deepcopy(nn.LayerNorm(d_model)) )
            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 2
            enc_output_norm_temp.append( copy.deepcopy(nn.LayerNorm(d_model)) )
            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 3
            enc_output_norm_temp.append( copy.deepcopy(nn.LayerNorm(d_model)) )
            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 4
            enc_output_norm_temp.append( copy.deepcopy(nn.LayerNorm(d_model)) )
            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 5
            enc_output_norm_temp.append( copy.deepcopy(nn.LayerNorm(d_model)) )
            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 6
            enc_output_norm_temp.append( copy.deepcopy(nn.LayerNorm(d_model)) )
            self.enc_output = nn.ModuleList( enc_output_temp )
            self.enc_output_norm = nn.ModuleList( enc_output_norm_temp )
            del enc_output_temp, enc_output_norm_temp
        ...
        enc_outputMem_LinearProjection = []
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) ) # 1
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) ) # 2
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) ) # 3
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) ) # 4
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) ) # 5
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) ) # 6
        self.enc_outputMem_LinearProjection = nn.ModuleList( enc_outputMem_LinearProjection )
        del enc_outputMem_LinearProjection
        ...
        if rm_self_attn_layers is not None:
            ...
            for lid, dec_layer in enumerate(self.decoder[0].layers): # 1
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[1].layers): # 2
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[2].layers): # 3
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[3].layers): # 4
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[4].layers): # 5
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[5].layers): # 6
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
        ...
        self.decoder[0].rm_detach = rm_detach # 1
        self.decoder[1].rm_detach = rm_detach # 2
        self.decoder[2].rm_detach = rm_detach # 3
        self.decoder[3].rm_detach = rm_detach # 4
        self.decoder[4].rm_detach = rm_detach # 5
        self.decoder[5].rm_detach = rm_detach # 6

        # self.task_DetHead = None
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, task_DetHead=None):
        """
        Input:
        ...
        """
        # self.task_DetHead = task_DetHead
        # prepare input for encoder
        #print("\n----------------------------")
        #for i, src in enumerate(srcs):
            #print(f"src[{i}] shape: {src.shape}")
        #for i, mask in enumerate(masks):
            #print(f"mask[{i}] shape: {mask.shape}")
        #for i, pos_embed in enumerate(pos_embeds):
            #print(f"pos_embed[{i}] shape: {pos_embed.shape}")
        src_flatten = []
        mask_flatten = []
        lvl_pos_embed_flatten = []
        spatial_shapes = []
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            #print(f"Level {lvl}: src shape before flattening: {src.shape}")
            bs, c, h, w = src.shape
            spatial_shape = (h, w)
            ...
            #print(f"Level {lvl}: src shape after flattening: {src.shape}")
        ...
        #print(f"spacial_shapes: {spatial_shapes}")
        ...
        memory, enc_intermediate_output, enc_intermediate_refpoints = self.encoder(
                ...
                ref_token_coord=enc_refpoint_embed, task_DetHead=task_DetHead, # bs, nq, 4
                )
        #########################################################
        # End Encoder
        # - memory: bs, \sum{hw}, c
        ...
        if self.two_stage_type =='standard':
            ...
            # memory_list = []
            # memory_list.append( copy.deepcopy(memory) )
            # memory_list.append( copy.deepcopy(memory) )
            # memory_list.append( copy.deepcopy(memory) )

            memory = self.enc_outputMem_LinearProjection[task_DetHead]( memory )
            output_memory, output_proposals = gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes, input_hw)
            output_memory = self.enc_output_norm[task_DetHead](self.enc_output[task_DetHead](output_memory))

            ### NAD Added ###
            # temp_output_proposals = []
            # temp_output_memory = []
            # temp_output_memory.append( copy.deepcopy(output_memory) )  ## 0
            # temp_output_proposals.append( copy.deepcopy(output_proposals) )
            # temp_output_memory.append( copy.deepcopy(output_memory) )  ## 1
            # temp_output_proposals.append( copy.deepcopy(output_proposals) )
            # temp_output_memory.append( copy.deepcopy(output_memory) )  ## 2
            # temp_output_proposals.append( copy.deepcopy(output_proposals) )
            # output_memory = temp_output_memory
            # output_proposals = temp_output_proposals
            # del temp_output_proposals, temp_output_memory
            ...
            #### NAD: Most likely need multiple heads here too -- DINO-> enc_out_class_embed | enc_out_bbox_embed
            # enc_outputs_class_unselected = self.enc_out_class_embed(output_memory)
            # enc_outputs_coord_unselected = self.enc_out_bbox_embed(output_memory) + output_proposals # (bs, \sum{hw}, 4) unsigmoid
            enc_outputs_class_unselected = self.enc_out_class_embed[task_DetHead](output_memory)
            enc_outputs_coord_unselected = self.enc_out_bbox_embed[task_DetHead](output_memory) + output_proposals # (bs, \sum{hw}, 4) unsigmoid

            topk = self.num_queries
            #if enc_outputs_class_unselected.max(-1)[0].size(1) < topk:
                #topk = enc_outputs_class_unselected.max(-1)[0].size(1)
            #num_elements = enc_outputs_class_unselected.max(-1)[0].size(1)
            #topk = min(self.num_queries, num_elements) # adjust topk to be in range of num_elements

            # print(f"--- DTF6: enc_outputs_class_unselected.max(-1)[0] shape: {enc_outputs_class_unselected.max(-1)[0].shape}")

            ### temporary fix ###
            #t = enc_outputs_class_unselected.max(-1)[0]
            #pad_size = topk - t.size(1)
            #if pad_size > 0:
                #padding = (0, 0) * (t.dim() - 1 - 1) + (0, pad_size)
                #t = torch.nn.functional.pad(t, padding, "constant", 0)

            #print(f"[AFTER PAD] enc_outputs_class_unselected.max(-1)[0] shape: {t.shape}")

            topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1] # bs, nq
            #topk_proposals = torch.topk(t, topk, dim=1)[1]
        ...
        #########################################################
        # Begin Decoder ---> NAD modified to have task_DetHead [1 Encoder but multiple Decoder part for multiple heads]
        #########################################################
        # print("[Check] Number of Decoder:",len(self.decoder))
        # print("[Check] tgt, memory:", tgt.shape, memory.shape)
        hs, references = self.decoder[task_DetHead](
                ...
                level_start_index=level_start_index, 
                spatial_shapes=spatial_shapes,
                valid_ratios=valid_ratios,tgt_mask=attn_mask, task_DetHead=task_DetHead)
        # print("[CHECK]", len(references))
        #########################################################
        # End Decoder
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
            ...
            ref_token_coord: Optional[Tensor]=None, task_DetHead=None 
            ):
        """
        Input:
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if ((layer_id == 0 and self.two_stage_type in ['enceachlayer', 'enclayer1']) \
                ...
                # gather boxes
                topk = self.num_queries
                # print("CHECK CHECK Number of class_embed", len(self.class_embed), len(self.class_embed[0]))
                enc_outputs_class = self.class_embed[layer_id](output_memory) ## NAD Modified for multi localization heads
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
                ...
                valid_ratios: Optional[Tensor] = None, task_DetHead=None,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.deformable_decoder: # default TRUE
            ...
            # iter update
            # print("CHECK CHECK CHECK 2 -- bbox_embed", len(self.bbox_embed))
            # print("CHECK CHECK CHECK 2 -- bbox_embed", self.bbox_embed)
            # if self.bbox_embed == None:
            #     print("CHECK CHECK -- TransformerDecoder -- FOUND self.bbox_embed to be NONE!")
            if self.bbox_embed is not None: ## NAD  -- CHECK THIS for multi Localizer Decoder Implementation
