...
def train_CLASSIFICATION(train_loader, model, criterion, optimizer, epoch, args, log_writter_CLASSIFICATION, head_number=None, model_ema=None, momen=None, coff=None, criterionMSE=None):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            ...
            out_classifierHead, features_cons = model.module.backbone[0].extra_features(images)
        else:
            # _, out_classifierHead, _ = model.backbone(images)
            out_classifierHead, features_cons = model.backbone[0].extra_features(images)
        ...
        if model_ema is not None:
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                _, features_cons_ema = model_ema.module.backbone[0].extra_features(images)
            else:
                _, features_cons_ema = model_ema.backbone[0].extra_features(images)
            loss_cons = criterionMSE(features_cons, features_cons_ema)
            loss = (1-coff)*loss + coff*loss_cons
...
def evaluate_CLASSIFICATION(val_loader, model, criterion, args, log_writter_CLASSIFICATION):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                # _, out_classifierHead, _ = model.module.backbone(images)
                out_classifierHead, _ = model.module.backbone[0].extra_features(images)
            else:
                # _, out_classifierHead, _ = model.backbone(images)
                out_classifierHead, _ = model.backbone[0].extra_features(images)
            output = out_classifierHead
...
def main(args):
    ...
    if args.taskcomponent in ["detect_segmentation_cyclic_v2", "detect_segmentation_cyclic_v3"]:
        loss_scaler = NativeScaler()
        param_dicts = get_param_dict(args, model_without_ddp)
        if args.opt == "adamw":
            optimizer = torch.optim.AdamW([
    ...
    for epoch in range(args.start_epoch, args.total_epochs):
        ...
        if args.taskcomponent == 'segmentation':
            ...
            lrLocalizer_ = step_decay(epoch, args.lr, args.total_epochs, step_inc=10)
            if len(optimizer.param_groups) == 2 or len(optimizer.param_groups) == 3:
                optimizer.param_groups[0]['lr'] = lrBackbone_
                optimizer.param_groups[1]['lr'] = lrSegmentor_
                optimizer.param_groups[2]['lr'] = lrLocalizer_                    
            ...
            train_lossAVG, _ = train_one_epoch_SEGMENTATION(model, train_loader, optimizer, loss_scaler, epoch, head_number=0, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
            ...
            test_y, test_p, _ = test_SEGMENTATION(model, test_loader, head_number=0, log_writter_SEGMENTATION=log_writter_SEGMENTATION)
            print( "Epoch {:04d}: val_loss {:.5f}.".format(epoch, val_avg_SEGMENTATION), file=log_writter_SEGMENTATION )
            print("[INFO] Test Dice = {:.6f}%.\n".format(100.0 * dice_score(test_p, test_y)), file=log_writter_SEGMENTATION)
            print( "Epoch {:04d}: val_loss {:.5f}.".format(epoch, val_avg_SEGMENTATION) )
            print("[INFO] Test Dice = {:.6f}%.\n".format(100.0 * dice_score(test_p, test_y)))
            # exit(0)
        ...
        if args.taskcomponent == "detect_segmentation_cyclic_v2":
            ...
            if args.debugOnlyTest == False:
                ...
                if TaskHead_number == 3: # Fine-tune for Localization - Left Lung
                    print()
                ...
                if TaskHead_number == 6: # Fine-tune for Localization - Right Lung
                    print()
