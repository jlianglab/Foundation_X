...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        decoder_.append( copy.deepcopy(decoder_temp) )
        decoder_.append( copy.deepcopy(decoder_temp) )
        decoder_.append( copy.deepcopy(decoder_temp) )
        ...
        if two_stage_type =='standard':
            ...
            enc_output_temp.append( copy.deepcopy(nn.Linear(d_model, d_model)) ) # 0
        ...
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) )
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) )
        enc_outputMem_LinearProjection.append( nn.Linear(d_model, d_model) )
        ...
        if rm_self_attn_layers is not None:
            ...
            for lid, dec_layer in enumerate(self.decoder[0].layers):
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[1].layers):
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
            for lid, dec_layer in enumerate(self.decoder[2].layers):
                if lid in rm_self_attn_layers:
                    dec_layer.rm_self_attn_modules()
        ...
        self.decoder[0].rm_detach = rm_detach
        self.decoder[1].rm_detach = rm_detach
        self.decoder[2].rm_detach = rm_detach
