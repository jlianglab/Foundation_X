...
from models.load_weights_model import load_weights, load_weights_resume, load_weights_foundationX
import math
...
## Freeze unFreeze for Segmentation Tasks
...
def train_one_epoch_SEGMENTATION(model, train_loader, optimizer, loss_scaler, epoch, head_number=None, log_writter_SEGMENTATION=None, model_ema=None, momen=None, coff=None, criterionMSE=None):
    ...
    for idx, (img,mask) in enumerate(train_loader):
        ...
        if model_ema is not None:
            ...
            loss_cons1 = criterionMSE(features_backbone, features_backbone_ema)
            loss_cons2 = criterionMSE(features_cons, features_cons_ema)
    ...
    return losses.avg, lossesDICE.avg, (lossesCONS1.avg+lossesCONS2.avg)/2, model_ema
...
def evaluate_CLASSIFICATION(val_loader, model, criterion, args, log_writter_CLASSIFICATION, head_number=None, task_cls_type=None):
    batch_time = AverageMeter_CLASSIFICATION('Time', ':6.3f')
    losses = AverageMeter_CLASSIFICATION('Loss', ':.4e')
    top1 = AverageMeter_CLASSIFICATION('Acc@1', ':6.2f')
    top5 = AverageMeter_CLASSIFICATION('Acc@5', ':6.2f')
...
def main(args):
    ...
    if args.taskcomponent in ["segmentation"]: # 
        args.num_classes = 3+1
        args.dn_labelbook_size = 5
    if args.taskcomponent in ["detect_node21_nodule", "detect_tbx11k_catagnostic", "ClsLoc_tbx11k_catagnostic","ClsLoc_rsna_pneumonia",'ClsLoc_node21_nodule','ClsLoc_candid_ptx','ClsLoc_SIIM_ACR']: # Binary Class
        args.num_classes = 2
        args.dn_labelbook_size = 3
    ...
    if args.taskcomponent in ["detect_chestxdet_dataset",'ClsLoc_chestxdet_dataset']: 
        args.num_classes = 14
        args.dn_labelbook_size = 15
    ...
    ### ----------------- Optimizer ----------------- ###
    if args.taskcomponent in ["detect_segmentation_cyclic", "detect_segmentation_cyclic_v4", "segmentation", 'detect_vindrcxr_heart_segTest', 'detect_vindrcxr_heart', 'detect_vindrcxr_leftlung', 'detect_vindrcxr_rightlung']:
    ...
    if args.taskcomponent in ["detect_segmentation_cyclic_v2", "detect_segmentation_cyclic_v3", "segmentation_vindrcxr_organ_1h3ch"]:
        loss_scaler = NativeScaler()
        param_dicts = get_param_dict(args, model_without_ddp)
        if args.opt == "adamw":
            optimizer = torch.optim.AdamW([
    ...
    if args.taskcomponent in ['detection', 'detection_baseline', 'detect_node21_nodule', 'detect_tbx11k_catagnostic', 'ClsLoc_tbx11k_catagnostic', 'detection_vindrcxr_disease', 'detection_vindrmammo_disease', 'detect_chestxdet_dataset',"ClsLoc_rsna_pneumonia",'ClsLoc_node21_nodule','ClsLoc_candid_ptx','ClsLoc_SIIM_ACR','ClsLoc_chestxdet_dataset']:
    ...
    if args.taskcomponent == "detect_segmentation_cyclic_v2": ## With EMA -- Teacher-Student Model
    ...
    if args.foundationX is not None:
        ...
        model, model_ema = load_weights_foundationX(model, model_ema, optimizer, args, to_load='model')
    ...
    if args.modelEMA == "True_Epoch":
        ...
        elif args.taskcomponent == 'detect_segmentation_cyclic_v2':
            Num_EPOCH_Iterative_Steps_MomentumSchduler = args.total_epochs
        else: 
...
##################### I have to add 5 other dataset########################################## 
        if args.taskcomponent in ['detection_vindrcxr_disease', 'detect_node21_nodule', 'detection_vindrmammo_disease', 'ClsLoc_tbx11k_catagnostic','ClsLoc_rsna_pneumonia','ClsLoc_node21_nodule','ClsLoc_candid_ptx','ClsLoc_SIIM_ACR','ClsLoc_chestxdet_dataset']: # VinDrCXR_Disease # NODE21_Nodule
            if args.taskcomponent == 'detection_vindrcxr_disease':
                datasetname__ = 'VinDrCXR_Disease'
            elif args.taskcomponent == 'detect_node21_nodule' or args.taskcomponent == 'ClsLoc_node21_nodule':
            ...
            elif args.taskcomponent == "ClsLoc_rsna_pneumonia":
                datasetname__ = "rsna_pneumonia"
            elif args.taskcomponent == "ClsLoc_candid_ptx":
                datasetname__ = "candid_ptx"
            elif args.taskcomponent == "ClsLoc_SIIM_ACR":
                datasetname__ = "SIIM_ACR"
            elif args.taskcomponent == "ClsLoc_chestxdet_dataset":
                datasetname__ = "chestxdet_dataset"
            ...
            if args.cyclictask == 'loc' and taskhead == 1:
        ...
        if args.taskcomponent == 'segmentation' or args.taskcomponent == "segmentation_vindrcxr_organ_1h3ch":
            ...
            for cls_value in range(0, args.numClasses):
                print("[INFO] Test Class {} Dice = {:.6f}%.\n".format(cls_value, 100.0 * dice_score(test_p[:,cls_value,:,:], test_y[:,cls_value,:,:])), file=log_writter_SEGMENTATION)
                print("[INFO] Test Class {} Dice = {:.6f}%.\n".format(cls_value, 100.0 * dice_score(test_p[:,cls_value,:,:], test_y[:,cls_value,:,:])))
            # exit(0)
        ...
        if args.taskcomponent == "detect_segmentation_cyclic_v2":
            ...
            if args.cyclictask == 'heart':
                ...
                if TaskHead_number in [2,5,8]: ## Ignore
                    continue
            ...
            if args.debugOnlyTest == False:
                ...
                if TaskHead_number == 1: # Fine-tune for Localization - Heart
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 0
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch(
                        model, criterion, data_loader_trainHeartB, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=1, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    log_stats = { **{f'train_Heart_{k}': v for k, v in train_stats.items()} }
                    result_output_dir = args.output_dir + '/results.txt'
                    log_writer_detection = open(result_output_dir, 'a')
                ...
                if TaskHead_number == 2: # Fine-tune for Segmentation - Heart || head_number = segmentation head number
                    ...
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_vindrcxrHeart_A, optimizer, loss_scaler, epoch, head_number=0, log_writter_SEGMENTATION=log_writter_SEGMENTATION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train HeartA Loss {:.5f} {:.5f} {:.5f} ".format(epoch, train_lossAVG, train_lossDiceAvg, train_lossConsAvg), file=log_writter_SEGMENTATION )
                if TaskHead_number == 3: # Fine-tune for Segmentation - Heart || head_number = segmentation head number
                    ...
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_vindrcxrHeart_B, optimizer, loss_scaler, epoch, head_number=0, log_writter_SEGMENTATION=log_writter_SEGMENTATION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train HeartB Loss {:.5f} {:.5f} {:.5f} ".format(epoch, train_lossAVG, train_lossDiceAvg, train_lossConsAvg), file=log_writter_SEGMENTATION )
                ...
                if TaskHead_number == 5: # Fine-tune for Localization - Left Lung
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 1
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, data_loader_trainLeftLungB, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=2, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    log_stats = { **{f'train_LeftLung_{k}': v for k, v in train_stats.items()} }
                    result_output_dir = args.output_dir + '/results.txt'
                    log_writer_detection = open(result_output_dir, 'a')
                ...
                if TaskHead_number == 6: # Fine-tune for Segmentation - Left Lung || head_number = segmentation head number
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 1
                    model = Freeze_Backbone_SegmentationDecoder(model)
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_vindrcxrLeftLung_A, optimizer, loss_scaler, epoch, head_number=1, log_writter_SEGMENTATION=log_writter_SEGMENTATION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Left LungA Loss {:.5f} {:.5f} {:.5f} ".format(epoch, train_lossAVG, train_lossDiceAvg, train_lossConsAvg), file=log_writter_SEGMENTATION )
                if TaskHead_number == 7: # Fine-tune for Segmentation - Left Lung || head_number = segmentation head number
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 1
                    model = unFreeze_Backbone_and_Localization_Encoder(model)
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_vindrcxrLeftLung_B, optimizer, loss_scaler, epoch, head_number=1, log_writter_SEGMENTATION=log_writter_SEGMENTATION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Left LungB Loss {:.5f} {:.5f} {:.5f} ".format(epoch, train_lossAVG, train_lossDiceAvg, train_lossConsAvg), file=log_writter_SEGMENTATION )
                ...
                if TaskHead_number == 9:
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 2
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, data_loader_trainRightLungB, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=3, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    log_stats = { **{f'train_RightLung{k}': v for k, v in train_stats.items()} }
                    result_output_dir = args.output_dir + '/results.txt'
                    log_writer_detection = open(result_output_dir, 'a')
                ...
                if TaskHead_number == 10: # Fine-tune for Segmentation - Right Lung || head_number = segmentation head number
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 2
                    model = Freeze_Backbone_SegmentationDecoder(model)
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_vindrcxrRightLung_A, optimizer, loss_scaler, epoch, head_number=2, log_writter_SEGMENTATION=log_writter_SEGMENTATION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Right LungA Loss {:.5f} {:.5f} {:.5f} ".format(epoch, train_lossAVG, train_lossDiceAvg, train_lossConsAvg), file=log_writter_SEGMENTATION )
                if TaskHead_number == 11: # Fine-tune for Segmentation - Right Lung || head_number = segmentation head number
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 2
                    model = unFreeze_Backbone_and_Localization_Encoder(model)
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_vindrcxrRightLung_B, optimizer, loss_scaler, epoch, head_number=2, log_writter_SEGMENTATION=log_writter_SEGMENTATION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Right LungB Loss {:.5f} {:.5f} {:.5f} ".format(epoch, train_lossAVG, train_lossDiceAvg, train_lossConsAvg), file=log_writter_SEGMENTATION )
        ...
        if args.taskcomponent == "detect_segmentation_cyclic_v3": ### Training Strategy 2
            TaskHead_number = (epoch - 1) % 9 # was 2 or 6
            if args.modelEMA:
                model_ema.eval()
            # EMA Update Epoch-wise
            if args.modelEMA == "True_Epoch":
            ...
            if args.cyclictask == 'heart':
                ...
                check = 0 ## Ignore Nothing as Current code doesn't have segmentation task
            elif args.cyclictask == 'heart_leftlung_rightlung':
                check = 0 ## Ignore nothing
            else:
                print("[W A R N N I N G] Cyclic Task Error! ", args.cyclictask)
            ...
            if TaskHead_number == 0: # Fine-tune for Localization - Heart PART1
                print()
                print("Localization Training: Heart A/Part1 [Freeze Backbone & Loc.Enc]") ## file=log_writter_SEGMENTATION
                print("Localization Training: Heart A/Part1 [Freeze Backbone & Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_FreezeBackbone_FreezeLocEnc_HeartA_Train"
                model = Freeze_Backbone(model)
                model = Freeze_Localization_Encoder(model)
                model.task_DetHead = 0
                if args.modelEMA:
                    model_ema.task_DetHead = 0
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainHeartA, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=1, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_HeartA_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: HeartA " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_HeartA_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            if TaskHead_number == 1: # Fine-tune for Localization - LeftLung Part1
                print()
                print("Localization Training: LeftLung A/Part1 [Freeze Backbone & Loc.Enc]")
                print("Localization Training: LeftLung A/Part1 [Freeze Backbone & Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_FreezeBackbone_FreezeLocEnc_LeftLungA_Train"
                model = Freeze_Backbone(model)
                model = Freeze_Localization_Encoder(model)
                model.task_DetHead = 1
                if args.modelEMA:
                ...
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainLeftLungA, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=2, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_LeftLungA_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: LeftLungA " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_LeftLungA_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            if TaskHead_number == 2: # Fine-tune for Localization - RightLung Part1
                print()
                print("Localization Training: RightLung A/Part1 [Freeze Backbone & Loc.Enc]")
                print("Localization Training: RightLung A/Part1 [Freeze Backbone & Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_FreezeBackbone_FreezeLocEnc_RightLungA_Train"
                model = Freeze_Backbone(model)
                model = Freeze_Localization_Encoder(model)
                model.task_DetHead = 2
                if args.modelEMA:
                ...
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainRightLungA, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=3, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_RightLungA_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: RightLungA " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_RightLungA_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            ...
            if TaskHead_number == 3: # Fine-tune for Localization - Heart PART2
                print()
                print("Localization Training: Heart B/Part1 [Freeze Backbone | Trainable Loc.Enc]")
                print("Localization Training: Heart B/Part1 [Freeze Backbone | Trainable Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_FreezeBackbone_TrLocEnc_HeartB_Train"
                model = Freeze_Backbone(model)
                model = unFreeze_Localization_Encoder(model)
                model.task_DetHead = 0
                if args.modelEMA:
                    model_ema.task_DetHead = 0
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainHeartB, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=1, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_HeartB_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: HeartB " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_HeartB_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            if TaskHead_number == 4: # Fine-tune for Localization - LeftLung Part2
                print()
                print("Localization Training: LeftLung B/Part1 [Freeze Backbone | Trainable Loc.Enc]")
                print("Localization Training: LeftLung B/Part1 [Freeze Backbone | Trainable Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_FreezeBackbone_TrLocEnc_LeftLungB_Train"
                model = Freeze_Backbone(model)
                model = unFreeze_Localization_Encoder(model)
                model.task_DetHead = 1
                if args.modelEMA:
                ...
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainLeftLungB, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=2, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_LeftLungB_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: LeftLungB " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_LeftLungB_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            if TaskHead_number == 5: # Fine-tune for Localization - RightLung Part2
                print()
                print("Localization Training: RightLung B/Part1 [Freeze Backbone | Trainable Loc.Enc]")
                print("Localization Training: RightLung B/Part1 [Freeze Backbone | Trainable Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_FreezeBackbone_TrLocEnc_RightLungB_Train"
                model = Freeze_Backbone(model)
                model = unFreeze_Localization_Encoder(model)
                model.task_DetHead = 2
                if args.modelEMA:
                ...
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainRightLungB, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=3, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_RightLungB_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: RightLungB " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_RightLungB_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            # if TaskHead_number == 5: # Fine-tune for Segmentation - Left Lung || head_number = segmentation head number
            ...
            if TaskHead_number == 6: # Fine-tune for Localization - Heart PART3
                print()
                print("Localization Training: Heart C/Part1 [Trainable Backbone | Trainable Loc.Enc]")
                print("Localization Training: Heart C/Part1 [Trainable Backbone | Trainable Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_TrBackbone_TrLocEnc_HeartC_Train"
                ...
                if args.modelEMA:
                    model_ema.task_DetHead = 0
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainHeartC, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=1, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_HeartC_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: HeartC " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_HeartC_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            if TaskHead_number == 7: # Fine-tune for Localization - LeftLung Part3
                print()
                print("Localization Training: LeftLung C/Part1 [Trainable Backbone | Trainable Loc.Enc]")
                print("Localization Training: LeftLung C/Part1 [Trainable Backbone | Trainable Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_TrBackbone_TrLocEnc_LeftLungC_Train"
                model = unFreeze_Backbone(model)
                model = unFreeze_Localization_Encoder(model)
                model.task_DetHead = 1
                if args.modelEMA:
                    model_ema.task_DetHead = 1
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainLeftLungC, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=2, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_LeftLungC_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: LeftLungC " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_LeftLungC_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            if TaskHead_number == 8: # Fine-tune for Localization - RightLung Part3
                print()
                print("Localization Training: RightLung C/Part1 [Trainable Backbone | Trainable Loc.Enc]")
                print("Localization Training: RightLung C/Part1 [Trainable Backbone | Trainable Loc.Enc]", file=log_writter_SEGMENTATION)
                task_todo = "Localization_TrBackbone_TrLocEnc_RightLungC_Train"
                model = unFreeze_Backbone(model)
                model = unFreeze_Localization_Encoder(model)
                model.task_DetHead = 2
                if args.modelEMA:
                    model_ema.task_DetHead = 2
                train_stats, model_ema = train_one_epoch(
                    model, criterion, data_loader_trainRightLungC, optimizer, device, epoch,
                    args.clip_max_norm, wo_class_error=wo_class_error, DetHead=3, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                log_stats = { **{f'train_RightLungC_{k}': v for k, v in train_stats.items()} }
                result_output_dir = args.output_dir + '/results.txt'
                log_writer_detection = open(result_output_dir, 'a')
                log_writer_detection.write('Epoch: ' + str(epoch) + "| TaskHead_number: " + str(TaskHead_number) + " Training: RightLungC " + '\n')
                log_writer_detection.write('-- Training --' + '\n')
                formatted_stats_train = {f'train_RightLungC_{k}': v for k, v in train_stats.items()}
                for key, value in formatted_stats_train.items():
                    log_writer_detection.write(f'{key}: {value}\n')
                log_writer_detection.write('\n')
                log_writer_detection.close()
            # if TaskHead_number == 8: # Fine-tune for Segmentation - Right Lung || head_number = segmentation head number
        ...
        if args.taskcomponent == "foundation_x_pretraining":
            ...
            if args.cyclictask == 'loc' or args.cyclictask == 'TESTloc_node21LOC' or args.cyclictask == 'TESTloc_tbx11kLOC': # 
                ...
                if TaskHead_number in [0,2,4,6,7,9,11,12,13,14]: ## Ignore ## Ignore SIIMacrPTX localization
                    continue
            elif args.cyclictask == 'seg' or args.cyclictask == 'TESTseg_candidptxSEG' or args.cyclictask == 'TESTseg_chestxdetSEG' or args.cyclictask == 'TESTseg_siimacrSEG':
            ...
            if args.debugOnlyTest == False:
                ...
                if TaskHead_number == 1: # Train for TBX11k Localization
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 0
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, train_loader_loc_TBX11k_B, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=0, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                ...
                if TaskHead_number == 3: # Train for NODE21 Localization
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 1
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, train_loader_loc_Node21_B, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=1, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                ...
                if TaskHead_number == 5: # Train for CANDID-PTX Localization
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 2
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, train_loader_loc_CANDIDptx_B, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=2, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                ...
                if TaskHead_number == 6: # Train for CANDID-PTX Segmentation
                    print()
                    print("Segmentation_CANDIDPTX_Train")
                    task_todo = "Segmentation_CANDIDPTX_Train"
                    model.task_DetHead = 2
                    if args.modelEMA:
                        model_ema.task_DetHead = 2
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_seg_CANDIDptx, optimizer, loss_scaler, epoch, head_number=2, log_writter_SEGMENTATION=log_writter_DETECTION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Segmentation CANDIDPTX Loss {:.5f} ".format(epoch, train_lossAVG), file=log_writter_DETECTION )
                    fields=[epoch, 'CANDID-PTX', task_todo, '-', '-', str(train_lossDiceAvg), str(train_lossConsAvg)] # AUC_SliceLevel_Res
                    with open(args.output_dir+'/export_csvFile_TRAIN.csv', 'a') as f: # export_csvFile_train = pd.DataFrame(columns=['Epoch', 'Dataset', 'Task-Train', 'Cls_Loss', 'Loc_Loss','Seg_Loss'])
                        writer = csv.writer(f)
                        writer.writerow(fields)
                ...
                if TaskHead_number == 7: # Train for RSNA Pneumonia Classification
                    ...
                    print( "Epoch {:04d}: Train Classification RSNA_Pneumonia Loss {:.5f} ".format(epoch, train_lossAVG) )
                    print( "Epoch {:04d}: Train Classification RSNA_Pneumonia Loss {:.5f} ".format(epoch, train_lossAVG), file=log_writter_DETECTION )
                ...
                if TaskHead_number == 8:  # Train for RSNA Pneumonia Localization
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 3
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, train_loader_loc_RSNApneumonia_B, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=3, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                ...
                if TaskHead_number == 10:  # Train for ChestX-Det Localization
                    ...
                    train_stats, model_ema = train_one_epoch(
                        ...
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=4, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                ...
                if TaskHead_number == 11: # Train for ChestX-Det Segmentation
                    print()
                    print("Segmentation_ChestXDet_Train")
                    task_todo = "Segmentation_ChestXDet_Train"
                    model.task_DetHead = 4
                    if args.modelEMA:
                        model_ema.task_DetHead = 4
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_seg_ChestXDet, optimizer, loss_scaler, epoch, head_number=4, log_writter_SEGMENTATION=log_writter_DETECTION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Segmentation ChestXDet Loss {:.5f} ".format(epoch, train_lossAVG), file=log_writter_DETECTION )
                    fields=[epoch, 'ChestX-Det', task_todo, '-', '-', str(train_lossDiceAvg), str(train_lossConsAvg)] # AUC_SliceLevel_Res
                    with open(args.output_dir+'/export_csvFile_TRAIN.csv', 'a') as f: # export_csvFile_train = pd.DataFrame(columns=['Epoch', 'Dataset', 'Task-Train', 'Cls_Loss', 'Loc_Loss','Seg_Loss'])
                        writer = csv.writer(f)
                        writer.writerow(fields)
                ...
                if TaskHead_number == 13:  # Train for SIIM_ACR_Pneumothorax Localization
                    ...
                    train_stats, model_ema = train_one_epoch(
                        ...
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=5, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    ...
                    if args.modelEMA:
                        model_ema.task_DetHead = 5
                    train_stats, model_ema = train_one_epoch(
                        model, criterion, train_loader_loc_SiimACR_B, optimizer, device, epoch,
                        args.clip_max_norm, wo_class_error=wo_class_error, DetHead=5, lr_scheduler=None, args=args, logger=(logger if args.save_log else None), model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                ...
                if TaskHead_number == 14: # Train for SIIMACR Segmentation
                    print()
                    print("Segmentation_SIIMACR_Train")
                    task_todo = "Segmentation_SIIMACR_Train"
                    model.task_DetHead = 5
                    if args.modelEMA:
                        model_ema.task_DetHead = 5
                    train_lossAVG, train_lossDiceAvg, train_lossConsAvg, model_ema = train_one_epoch_SEGMENTATION(model, train_loader_seg_SIIM, optimizer, loss_scaler, epoch, head_number=5, log_writter_SEGMENTATION=log_writter_DETECTION, model_ema=model_ema, momen=momentum_schedule, coff=coff, criterionMSE=criterionMSE)
                    print( "Epoch {:04d}: Train Segmentation SIIMACR Loss {:.5f} ".format(epoch, train_lossAVG), file=log_writter_DETECTION )
                    fields=[epoch, 'SIIM-ACR', task_todo, '-', '-', str(train_lossDiceAvg), str(train_lossConsAvg)] # AUC_SliceLevel_Res
                    with open(args.output_dir+'/export_csvFile_TRAIN.csv', 'a') as f: # export_csvFile_train = pd.DataFrame(columns=['Epoch', 'Dataset', 'Task-Train', 'Cls_Loss', 'Loc_Loss','Seg_Loss'])
                        writer = csv.writer(f)
                        writer.writerow(fields)
            ...
            elif 'TESTcls' in args.cyclictask and 'node21CLS' in args.cyclictask:
                datasetname__ = "Node21"
                head_number_temp = 1
                task_cls_type_temp = None
                test_loader_cls_temp = test_loader_cls_NODE21
            elif 'TESTcls' in args.cyclictask and 'candidptxCLS' in args.cyclictask:
                datasetname__ = "CANDID-PTX"
                head_number_temp = 2
                task_cls_type_temp = 'nonBinary'
                test_loader_cls_temp = test_loader_cls_CANDIDptx
            elif 'TESTcls' in args.cyclictask and 'rsnapneumoniaCLS' in args.cyclictask:
                datasetname__ = "RSNApneumonia"
                head_number_temp = 3
                task_cls_type_temp = 'nonBinary'
                test_loader_cls_temp = test_loader_cls_RSNApneumonia
            elif 'TESTcls' in args.cyclictask and 'chestxdetCLS' in args.cyclictask:
                datasetname__ = "ChestX-Det"
                head_number_temp = 4
                task_cls_type_temp = 'nonBinary'
                test_loader_cls_temp = test_loader_cls_ChestXDet
            elif 'TESTcls' in args.cyclictask and 'siimacrCLS' in args.cyclictask:
            ...
            elif 'TESTloc' in args.cyclictask and 'node21LOC' in args.cyclictask:
                datasetname__ = "Node21"
                model.task_DetHead = 1
                model_ema.task_DetHead = 1
                DetHead_temp = 1
            ...
            elif 'TESTloc' in args.cyclictask and 'candidptxLOC' in args.cyclictask:
                datasetname__ = "CANDID-PTX"
                model.task_DetHead = 2
                model_ema.task_DetHead = 2
                DetHead_temp = 2
            ...
            elif 'TESTloc' in args.cyclictask and 'rsnapneumoniaLOC' in args.cyclictask:
                datasetname__ = "RSNApneumonia"
                model.task_DetHead = 3
                model_ema.task_DetHead = 3
                DetHead_temp = 3
            ...
            elif 'TESTloc' in args.cyclictask and 'chestxdetLOC' in args.cyclictask:
                datasetname__ = "ChestX-Det"
                model.task_DetHead = 4
                model_ema.task_DetHead = 4
                DetHead_temp = 5
            ...
            elif 'TESTloc' in args.cyclictask and 'siimacrLOC' in args.cyclictask:
                datasetname__ = "SIIM-ACRpneumothorax"
                model.task_DetHead = 5
                model_ema.task_DetHead = 5
                DetHead_temp = 5
            ...
            elif 'TESTseg' in args.cyclictask and 'chestxdetSEG' in args.cyclictask:
                datasetname__ = "ChestX-Det"
                head_number_temp = 4
                test_loader_loc_temp = test_loader_seg_ChestXDet
            elif 'TESTseg' in args.cyclictask and 'siimacrSEG' in args.cyclictask:
                datasetname__ = "SIIM-ACRpneumothorax"
                head_number_temp = 5
