...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        if (two_stage_type != 'no' and embed_init_tgt) or (two_stage_type == 'no'):
            self.tgt_embed_ = nn.Embedding(self.num_queries, d_model)
            nn.init.normal_(self.tgt_embed_.weight.data)
            self.tgt_embed = []
            self.tgt_embed.append( copy.deepcopy(self.tgt_embed_) )
            self.tgt_embed.append( copy.deepcopy(self.tgt_embed_) )
            self.tgt_embed.append( copy.deepcopy(self.tgt_embed_) )
            self.tgt_embed = nn.ModuleList( self.tgt_embed )
        else:
            self.tgt_embed = None
        ...
        if two_stage_type =='standard':
            ...
            self.enc_output_ = nn.Linear(d_model, d_model)
            self.enc_output_norm_ = nn.LayerNorm(d_model) 

            self.enc_output = []
            self.enc_output_norm = []
            self.enc_output.append( copy.deepcopy(self.enc_output_) )
            self.enc_output.append( copy.deepcopy(self.enc_output_) )
            self.enc_output.append( copy.deepcopy(self.enc_output_) )

            self.enc_output_norm.append( copy.deepcopy(self.enc_output_norm_) )
            self.enc_output_norm.append( copy.deepcopy(self.enc_output_norm_) )
            self.enc_output_norm.append( copy.deepcopy(self.enc_output_norm_) )
            self.enc_output = nn.ModuleList( self.enc_output )
            self.enc_output_norm = nn.ModuleList( self.enc_output_norm ) 
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, task_DetHead=0):
        """
        Input:
        ...
        if self.two_stage_type =='standard':
            if self.two_stage_learn_wh:
                ...
                input_hw = None
            # print("[Check] DefTransformer - TaskDetHead", task_DetHead)
            output_memory, output_proposals = gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes, input_hw)
            output_memory = self.enc_output_norm[task_DetHead](self.enc_output[task_DetHead](output_memory)) # NAD modified
            ...
            # print("[Check] DefTransformer - TaskDetHead", task_DetHead)
            enc_outputs_class_unselected = self.enc_out_class_embed[task_DetHead](output_memory) ## NAD modified
            enc_outputs_coord_unselected = self.enc_out_bbox_embed[task_DetHead](output_memory) + output_proposals # (bs, \sum{hw}, 4) unsigmoid # NAD modified
            # print("[Check] enc_outputs_class_unselected ClassEmbed", enc_outputs_class_unselected.shape)
            # print("[Check] enc_outputs_coord_unselected BboxEmbed", enc_outputs_coord_unselected.shape)
            topk = self.num_queries
            topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1] # bs, nq
            # print("[Check] topk_proposals topk", topk_proposals.shape)
            ...
            # print("[Check] refpoint_embed_ gather", refpoint_embed_.shape)
            init_box_proposal = torch.gather(output_proposals, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 4)).sigmoid() # sigmoid
            # print("[Check] init_box_proposal gather", init_box_proposal.shape)
        ...
        # Begin Decoder
        #########################################################
        # print("[Check] DefTransformer Before decoder - TaskDetHead", task_DetHead)
        ...
        # print("[Check] DefTransformer After decoder - TaskDetHead", task_DetHead)
        #########################################################
        # End Decoder
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            # iter update
            # print("[CHECK] self.bbox_embed", len(self.bbox_embed), len(self.bbox_embed[0]) )
            # print(self.bbox_embed)
            # print("[Check Def.Trans] task_DetHead", task_DetHead)
            if self.bbox_embed[task_DetHead] is not None:
                reference_before_sigmoid = inverse_sigmoid(reference_points)
                delta_unsig = self.bbox_embed[task_DetHead][layer_id](output)
                ...
                if self.dec_layer_number is not None and layer_id != self.num_layers - 1:
                    ...
                    if nq_now != select_number:
                        class_unselected = self.class_embed[task_DetHead][layer_id](output) # nq, bs, 91
