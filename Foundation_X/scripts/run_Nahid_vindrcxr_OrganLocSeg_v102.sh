#!/bin/bash
#SBATCH --job-name=SLURM_SwinB_IntegratedModel_run101_Shikhar_VinDrCXR_HLLRLsegmentation
...
LOGFILE=/scratch/nuislam/Model_Checkpoints/IntegratedModel_DINOpipeline/Integrated_Model_vindrcxr_OrganSeg_/run105_Nahid_VinDrCXR_HLLRLSegmentation_FuF_m0.80_noLayerNorm_yesFeatureNorm_noL2Reg_TrStg2_emaUpdateuF
...
opt=adamw
SERVER=SOL
cyclictask=heart_leftlung_rightlung_onlySeg
EMAMODE=True_Epoch
...
RESUME=/scratch/nuislam/Model_Checkpoints/IntegratedModel_DINOpipeline/Integrated_Model_vindrcxr_OrganSeg_/run105_Nahid_VinDrCXR_HLLRLSegmentation_FuF_m0.80_noLayerNorm_yesFeatureNorm_noL2Reg_TrStg2_emaUpdateuF/ckpt_E313_TH12.pth
...
export CUDA_VISIBLE_DEVICES=0 && ~/.conda/envs/tf-tnt-gpu2/bin/python main.py --taskcomponent detect_segmentation_cyclic_v3 --train --numClasses 1 --num_workers $num_workers --coco_path $coco_path --weight-decay 0.0001 \
	--output_dir $LOGFILE -c $CONFIGFILE --imgsize $IMGSIZE --backbonemodel $BACKBONEMODEL --init $INIT --total_epochs $total_epochs --batch_size $BATCHSIZE --opt $opt \
	--finetune_ignore label_enc.weight class_embed \
	--backbone_dir $backbone_dir --lr_backbone $lr_backbone --lr_locEnc $lr_locEnc --lr_locDec $lr_locDec  --lr_segmentor $lr_segmentor \
