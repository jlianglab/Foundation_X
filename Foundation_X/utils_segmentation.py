...
def save_model(model, optimizer, log_writter_SEGMENTATION, epoch, save_file, model_ema=None):
    print('==> Saving...',file=log_writter_SEGMENTATION)
    if model_ema is not None:
        state = {
            'model': model.state_dict(),
            'teacher_model': model_ema.state_dict(),
...
def save_model2(model, optimizerA, optimizerB,  log_writter_SEGMENTATION, epoch, save_file, model_ema=None):
    # optimizerA == ADAMW
    # optimizerB == SGD
    print('==> Saving...',file=log_writter_SEGMENTATION)
    if model_ema is not None:
        state = {
            'model': model.state_dict(),
            'teacher_model': model_ema.state_dict(),
            'optimizer_adamw': optimizerA.state_dict(),
            'optimizer_sgd': optimizerB.state_dict(),
            'epoch': epoch,
        }
    else:
        state = {
...
def mean_dice_coef(y_true,y_pred): ## using this.
    sum=0
...
def step_decay(step, start_lr, total_epoch, step_inc=20):

    lr = start_lr
    progress = (step - step_inc) / float(total_epoch - step_inc)
    progress = np.clip(progress, 0.0, 1.0)
    lr = lr * 0.5 * (1. + np.cos(np.pi * progress))

    lr = lr * np.minimum(1., step / step_inc)
