...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            avg_ap = 0.0
            if ap == 1:
                if iouThr == 0.4 or iouThr == 0.5:
                    num_classes = s.shape[2]
                    print("Individual Results for mAP", iouThr)
                    for i in range(0, num_classes):
                        print('category : {0} : {1}'.format(i,np.mean(s[:,:,i,:])))
                        avg_ap +=np.mean(s[:,:,i,:])
                    print('(all categories) mAP{} : {}'.format(iouThr, avg_ap / num_classes))

            return mean_s
        def _summarizeDets():
            stats = np.zeros((13,)) # was 12
            ...
            # stats[12] = _summarize(1, iouThr=.4, maxDets=self.params.maxDets[2])

            stats[0] = _summarize(1)
            stats[1] = _summarize(1, iouThr=.4, maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.4, 0.95, int(np.round((0.95 - .4) / .05)) + 1, endpoint=True)
